...
*/
let allHrsArray = Array.from(allHrs); // OR let allHrsArray = [...allHrs];
...
let test = Array.from(document.querySelectorAll(".image")).forEach(
  (val) => (val.style.border = "2px solid purple")
);
// Hide the box number 17 (last box).
document.querySelector(".seventeen").style.display = "none";
// Change the border of all the hr element from solid to dashed type
allHrsArray.forEach((val) => (val.style.border = "dashed"));
...
Array.from(document.querySelector(".one").children).forEach((val) =>
  val.remove()
);
...
para2.innerHTML = "<p>Append inserts as last child</p>";
...
para3.innerHTML = "<p>Prepend inserts as first child</p>";
...
for (let i = 0; i < allBoxes.length; i++) {
  if (i % 2 !== 0) {
    allBoxes[i].style.backgroundColor = "aliceblue";
  }
}
...
Array.from(seventh.children).forEach((val) => val.remove());
// Append the imgElm to the box no 7
seventh.append(imgElm);
